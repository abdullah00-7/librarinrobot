...
/**
 * the Packet class, for transmission over the bluetooth
 */
public class Packet {
	...
	public Packet(int sq, int command, String[] parameter) {
		...
		/**
		 * create the parameter while parameter is null
		 */
		if (parameter == null) {
	...
	private byte[] encode(String strPlainText) throws UnsupportedEncodingException {
		...
		System.arraycopy(new byte[] { 0x00 }, 0, ascTemp, ascTemp.length - 1, 1);
		return ascTemp;
	}
